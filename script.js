document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCountdown() {
        const eventDate = new Date('2025-02-25T11:00:00-06:00'); // 11am Costa Rica time
        const now = new Date();
        const diff = eventDate - now;

        if (diff <= 0) {
            document.getElementById('countdown').innerHTML = '<div class="countdown-item"><span>Event Started!</span></div>';
            return;
        }
