document.addEventListener("DOMContentLoaded", () => {
  // Track scroll depth for engagement tracking
  const scrollDepths = [25, 50, 75, 90];
  let trackedScrollDepths = [];
  ...
  const getRandomTags = (maxLength = 46) => {
    ...
    while (usedIndexes.size < tags.length) {
      ...
      const newLength =
        currentLength + tag.length + (result.length > 0 ? 2 : 0); // +2 for ", "
      ...
      if (
        result.length >= 2 &&
        tags[index].length + 2 > maxLength - currentLength
      )
        break;
    }
  ...
  document.querySelectorAll(".faq-question").forEach((question) => {
    question.addEventListener("click", () => {
      ...
      // Track FAQ interaction with Facebook Pixel
      if (typeof fbq !== 'undefined') {
        fbq('track', 'ViewContent', {
          content_name: 'FAQ',
          content_category: 'Course FAQ',
          content_ids: [questionText.substring(0, 30)]
        });
      }
