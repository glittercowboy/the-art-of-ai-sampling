...
describe("Next.js Conversion Features", () => {
  test("should render main page content", () => {
    ...
    expect(
      screen.getByText("IF YOU CAN IMAGINE IT, YOU CAN SAMPLE IT")
    ).toBeInTheDocument();
    expect(screen.getByText("THE ART OF A.I. SAMPLING")).toBeInTheDocument();
  });
  ...
  test("should have Facebook pixel integration", () => {
    ...
    const hasPixelScript = Array.from(scripts).some(
      (script) => script.innerHTML && script.innerHTML.includes("fbq")
    );
  ...
  test("should render FAQ section", () => {
    ...
    expect(
      screen.getByText("How is this different from other AI music courses?")
    ).toBeInTheDocument();
    expect(
      screen.getByText("What exactly will I learn in this course?")
    ).toBeInTheDocument();
  });
