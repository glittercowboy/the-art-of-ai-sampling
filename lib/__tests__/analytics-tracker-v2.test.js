  ...
  describe('Event Batching', () => {
    it('should batch events before sending', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('/api/analytics/batch', {
        ...
        body: JSON.stringify({
          events: [
            expect.objectContaining({ eventName: 'event1', properties: { data: 1 } }),
            expect.objectContaining({ eventName: 'event2', properties: { data: 2 } }),
            expect.objectContaining({ eventName: 'event3', properties: { data: 3 } })
          ]
        })
      })
    })
