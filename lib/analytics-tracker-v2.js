...
class AnalyticsTrackerV2 {
  constructor(config = {}) {
    this.config = {
      ...
      activeVisitorEndpoint: config.activeVisitorEndpoint || '/api/analytics/active-visitors',
  ...
  async sendHeartbeat() {
    try {
      const response = await fetch(this.config.activeVisitorEndpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (this.config.debug && response.ok) {
        ...
        console.log('[Analytics] Active visitors:', data.count)
      }
    } catch (error) {
      if (this.config.debug) {
